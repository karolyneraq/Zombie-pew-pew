...
class Hall2:
    ...
    def add_elements(self):
        ...
        for x in range(250, 851, 75):
            Element((x, 0), [self.obstacle], self.wall_top)

        Element((175, 0), [self.obstacle], self.wall_corner_real)

        # Left Walls
        wall_top_side_rotated = pygame.transform.rotate(self.wall_top_side, 90)
        Element((175, 75), [self.obstacle], self.wall_top_side)
        Element((175, 450), [self.obstacle], wall_top_side_rotated)

        # Lower Walls
        wall_corner_real_rotated_lower = pygame.transform.rotate(self.wall_corner_real, 90)
        Element((175, 525), [self.obstacle], wall_corner_real_rotated_lower)

        wall_top_inverted = pygame.transform.rotate(self.wall_top, 180)
        for x in range(250, 851, 75):
            Element((x, 525), [self.obstacle], wall_top_inverted)
        ...
        Element((325, 525), [self.obstacle], wall_door_top1_flip)
        Element((700, 525), [self.obstacle], wall_door_top1_rotated)
        ...
        # Stretchers
        Element((400, 375), [self.obstacle], self.stretcher_vertical1)
        Element((475, 375), [self.obstacle], self.stretcher_diagonal1)
